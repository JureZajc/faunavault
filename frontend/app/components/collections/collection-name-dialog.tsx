...
export default function CollectionNameDialog({ mode, kind = "Collection", initialName = "", isOpen, onClose, onSave }: {
  mode: "create" | "rename";
  ...
  async function submit(event: FormEvent) {
    ...
    if (isBusy) return;
    if (!name.trim()) { setError(`${kind} name must not be empty.`); return; }
    setIsBusy(true);
    setError(null);
    try {
      await onSave(name);
      ...
      setError(nextError instanceof Error ? nextError.message : `Could not save the ${kind}`);
    } finally {
      setIsBusy(false);
    }
  }

  const title = `${mode === "create" ? "Create" : "Rename"} ${kind}`;
  return <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-stone-950/40 p-3 sm:p-6">
    <form ref={dialogRef} onSubmit={submit} onKeyDown={handleKeyDown} role="dialog" aria-modal="true" aria-labelledby="collection-name-title" className="my-auto w-full max-w-md rounded-lg bg-white p-5 shadow-xl">
      ...
      <label className="mt-4 block text-sm font-medium text-stone-700">{kind} name
