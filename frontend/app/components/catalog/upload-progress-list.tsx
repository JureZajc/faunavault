...
export default function UploadProgressList({
  ...
  return (
    <section className="mt-4 border-t border-stone-200 pt-4" aria-labelledby="upload-progress-title">
          ...
          return (
            ...
            >
              <div className="flex min-w-0 flex-col gap-1 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
                <p className="min-w-0 truncate text-sm font-medium text-stone-900" title={item.filename}>
                  {item.filename}
                </p>
                <p className={`shrink-0 text-sm font-medium ${presentation.className}`}>
                ...
                <div className="mt-1 flex flex-wrap items-center justify-between gap-2">
                  ...
                  {item.failure.retryable ? (
                    <button
                      ...
                      className="min-h-9 rounded-md border border-red-200 bg-red-50 px-3 text-xs font-semibold text-red-800 transition hover:bg-red-100"
                    >
                      Retry
                    </button>
                  ) : null}
      ...
      {catalogRefreshError ? (
        <p role="alert" className="mt-3 rounded-md bg-amber-50 p-3 text-sm text-amber-800">
          {catalogRefreshError}
        </p>
      ) : null}
    </section>
