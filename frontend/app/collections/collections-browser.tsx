...
export default function CollectionsBrowser() {
  ...
  return <main className="min-h-screen bg-[#f7f8f4] text-stone-950">
    ...
    <CollectionNameDialog key={nameTarget === "create" ? "create" : nameTarget?.id ?? "closed"} mode={nameTarget === "create" ? "create" : "rename"} initialName={nameTarget && nameTarget !== "create" ? nameTarget.name : ""} isOpen={nameTarget !== null} onClose={() => setNameTarget(null)} onSave={saveName} />
    <DeleteCollectionDialog key={deleteTarget?.id ?? "closed"} collection={deleteTarget} onClose={() => setDeleteTarget(null)} onDelete={async () => { if (!deleteTarget) return; await deleteCollection(deleteTarget.id); collections.setItems((current) => current.filter((item) => item.id !== deleteTarget.id)); setNotice(`Deleted Collection “${deleteTarget.name}”. Its photos remain in FaunaVault.`); }} />
  </main>;
}
