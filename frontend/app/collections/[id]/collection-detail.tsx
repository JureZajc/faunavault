...
export default function CollectionDetailView({ collectionId }: { collectionId: number }) {
  ...
  return <main className="min-h-screen bg-[#f7f8f4] text-stone-950">
    ...
    <CollectionNameDialog key={renameOpen ? collection.updated_at : "closed"} mode="rename" initialName={collection.name} isOpen={renameOpen} onClose={() => setRenameOpen(false)} onSave={async (name) => { const saved = await renameCollection(collectionId, { name }); detail.setData((current) => current ? { ...current, ...saved } : current); setNotice(`Renamed Collection to “${saved.name}”.`); return saved; }} />
    <DeleteCollectionDialog key={deleteOpen ? collectionId : "closed"} collection={deleteOpen ? collection : null} onClose={() => setDeleteOpen(false)} onDelete={async () => { await deleteCollection(collectionId); router.push("/collections"); }} />
    <RemoveFromCollectionDialog key={removeIds?.join("-") ?? "closed"} collectionId={collectionId} collectionName={collection.name} photoIds={removeIds} onClose={() => setRemoveIds(null)} onSuccess={(count) => void removed(count)} />
  </main>;
}
