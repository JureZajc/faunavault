...
The lifespan also owns one reusable synchronous Ollama HTTP client. Constructing it performs no network request, so startup does not require Ollama or a configured model. Classification uses JSON-schema structured output, `think: false`, temperature zero, and the configured request-level keep-alive. Qwen3-VL on Ollama 0.32.15 may still place schema-valid metadata in the `thinking` response field; when the normal `response` field is empty, FaunaVault accepts that alternate channel only after the same strict validation and never logs its raw contents. Each primary or distinct fallback stage gets at most one automatic retry for explicitly transient transport, timeout, status, or malformed-response failures; internal requests remain part of one durable job attempt. A missing primary model skips its same-model retry, request/data errors do not fall back, and identical model names have no fallback stage.

Defaults are a 5-second connect timeout, 180-second read/request timeout, and `15m` keep-alive. These are loaded through `Settings` as `OLLAMA_CONNECT_TIMEOUT_SECONDS`, `OLLAMA_REQUEST_TIMEOUT_SECONDS`, and `OLLAMA_KEEP_ALIVE`; `0` keep-alive unloads immediately. The automatic retry pause is a fixed two seconds rather than another configuration knob. Successful Ollama duration/count metadata and sanitized failures are logged with job, Photo, model, role, and internal attempt context. Detailed timings and retry counts are not persisted.

An httpx timeout is a FaunaVault transport boundary. An HTTP error returned by Ollama—including a runner/model load timeout reported as HTTP 500—is an Ollama-side server failure and remains distinct in error handling and troubleshooting. FaunaVault never changes Ollama service, GPU, runner, or model-storage settings.
